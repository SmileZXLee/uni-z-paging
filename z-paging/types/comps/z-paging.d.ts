...
declare interface ZPagingProps {
   ...
   * @since 2.8.9
   */
  virtualParentLevel?: boolean

  /**
   * 父组件层级，默认为0，此属性为了解决vue3+(微信小程序或QQ小程序)中，scrollIntoViewById和scrollIntoViewByIndex因无法获取节点信息导致滚动到指定view无效的问题
   * - 仅vue3+(微信小程序或QQ小程序)且需要调用scrollIntoViewById或scrollIntoViewByIndex方法时有效，其他情况此属性设置任何值都无效。如果z-paging在swiper-item内，设置为1；如果嵌套层级更深，设置对应的数字(如2、3、4等)
   * @default 0
   * @since 2.8.9
   */
  parentLevel?: number | string
