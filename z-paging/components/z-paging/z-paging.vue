...
<template name="z-paging">
  <scroll-view
    @scrolltolower="_onLoadingMore('toBottom')"
    scroll-y="true"
    class="scroll-view"
  ...
  >
    <slot
      v-if="$slots.empty&&!totalData.length&&!hideEmptyView&&!firstPageLoaded&&!loading"
      name="empty"
    />
    <view
      v-else-if="emptyViewIs.length&&!totalData.length&&!hideEmptyView&&!firstPageLoaded&&!loading"
      :is="emptyViewIs"
    ></view>
    <slot />
    <slot
      @click="_onLoadingMore('click')"
      v-if="loadingStatus==0&&$slots.loadingMoreDefault&&showLoadingMore"
      name="loadingMoreDefault"
    />
    <slot
      @click="_onLoadingMore('click')"
      v-else-if="loadingStatus==1&&$slots.loadingMoreLoading&&showLoadingMore"
      name="loadingMoreLoading"
    />
    <slot
      @click="_onLoadingMore('click')"
      v-else-if="loadingStatus==2&&$slots.loadingMoreNoMore&&showLoadingMore"
      name="loadingMoreNoMore"
    />
    <slot
      @click="_onLoadingMore('click')"
      v-else-if="loadingStatus==3&&$slots.loadingMoreFail&&showLoadingMore"
      name="loadingMoreFail"
    />
    <view
      @click="_onLoadingMore('click')"
      v-else-if="showLoadingMore&&showDefaultLoadingMoreText"
      class="load-more-text"
      >{{ownLoadingMoreText}}</view
    >
  </scroll-view>
</template>
