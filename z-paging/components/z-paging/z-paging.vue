...
<template name="z-paging">
	<scroll-view @scrolltolower="_onLoadingMore" scroll-y="true" class="scroll-view" :enable-back-to-top="enableBackToTop" :show-scrollbar="showScrollbar" :refresher-enabled="refresherEnabled" :refresher-threshold="refresherThreshold" :refresher-triggered="refresherTriggered" @refresherrestore="_onRestore" @refresherrefresh="_onRefresh">
		<slot v-if="$slots.empty&&!totalData.length&&!hideEmptyView&&!firstPageLoaded&&!loading" name="empty" />
		<view v-else-if="emptyViewIs.length&&!totalData.length&&!hideEmptyView&&!firstPageLoaded&&!loading" :is="emptyViewIs"></view>
		<slot/>
		<slot @click="_onLoadingMore" v-if="loadingStatus==0&&$slots.loadingMoreDefault&&showLoadingMore" name="loadingMoreDefault" />
		<slot @click="_onLoadingMore" v-else-if="loadingStatus==1&&$slots.loadingMoreLoading&&showLoadingMore" name="loadingMoreLoading" />
		<slot @click="_onLoadingMore" v-else-if="loadingStatus==2&&$slots.loadingMoreNoMore&&showLoadingMore" name="loadingMoreNoMore" />
		<slot @click="_onLoadingMore" v-else-if="loadingStatus==3&&$slots.loadingMoreFail&&showLoadingMore" name="loadingMoreFail" />
		<view @click="_onLoadingMore" v-else-if="showLoadingMore&&showDefaultLoadingMoreText" class="load-more-text" >{{ownLoadingMoreText}}</view>
	</scroll-view>
</template>
...
<script>
	 ...
	 * @property {Boolean} loading-more-enabled 是否启用滑动到底部加载更多数据，默认为是
	 * @property {String} loading-more-default-text 滑动到底部加载"默认"文字，默认为【点击加载更多】
	 * @property {String} loading-more-loading-text 滑动到底部加载"加载中"文字，默认为【加载中...】
	 * @property {String} loading-more-no-more-text 滑动到底部加载"没有更多"文字，默认为【没有更多了】
	 * @property {String} loading-more-fail-text 滑动到底部加载"加载失败"文字，默认为【加载失败，点击重新加载】
	...
	export default {
		...
		data() {
			return {
				...
				loadingStatus: 0,
				//底部加载更多文字Map
				loadingStatusTextMap: {0:this.loadingMoreDefaultText,1:this.loadingMoreLoadingText,2:this.loadingMoreNoMoreText,3:this.loadingMoreFailText}
			};
		},
		props: {
			//自定义pageNo，默认为1
			...
			//滑动到底部加载"加载失败"文字，默认为【加载失败，点击重新加载】
			loadingMoreFailText: {
				type: String,
				default: function() {
					return '加载失败，点击重新加载'
			...
			//空数据图的组件名，无法使用easycom，建议通过：<view slot="empty">空数据内容</view>引入空数据图
			emptyViewIs: {
				type: String,
				default: function() {
					return ''
